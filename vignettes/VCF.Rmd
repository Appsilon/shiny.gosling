...
We can visualoze breakpoints with a circos plot.
...
We will have 3 tracks in this ciscos plot. The outermost layer displays the width of a sequence as a bar plot in maroon.

```{r, echo=TRUE, track-1, eval=FALSE}
...
```{r, echo=TRUE, track-2, eval=FALSE}
...
```{r, echo=TRUE, track-3, eval=FALSE}

track3 <- add_single_track(
  id = "track3",
  title = "Highlight Chr 10 to others",
  data = track_data_gr(
    gr.circos, chromosomeField = "to.gr.seqnames",
...
```{r, echo=TRUE, single_composed_track, eval=FALSE}
...
```{r, echo=TRUE, run_app, eval=FALSE}
...
```{r, echo=FALSE, run_app, eval=FALSE}
